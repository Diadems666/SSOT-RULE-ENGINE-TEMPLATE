...
async def generate_docs():
    ...
    try:
        ...
        if not rule_config:
            return jsonify({"success": False, "error": "No rule configuration provided"}), 400
...
async def enhance_memory():
    ...
    try:
        ...
        if not memory_entry:
            return jsonify({"success": False, "error": "No memory entry provided"}), 400
...
async def analyze_codebase():
    ...
    try:
        data = request.json
        if not data:
            return jsonify({"success": False, "error": "No analysis parameters provided"}), 400
...
async def update_context():
    ...
    try:
        data = request.get_json()
        if not data:
            return jsonify({"success": False, "error": "No context data provided"}), 400
...
async def search_kg():
    ...
    try:
        ...
        results = await kg_client.search(
            query=data['query'],
